...
- **Helper binary**: systray2 spawns a separate native binary, resolved as
  `./traybin/<name>` relative to the working directory first and
  `<module dir>/traybin/<name>` second. From `node_modules` that fallback lands
  inside the systray2 package, but in the bundled build it resolves next to the
  bundle, so the working-directory lookup is the one that matters: the installer
  ships `traybin/` beside the executable and registers the startup shortcut with
